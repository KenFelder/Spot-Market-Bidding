...
def update_limits(self, player, step_factor=0.1):
    ...
    if self.df_bidders.at[player, 'x_imb'] > 0:
        limit_sell = (1 - step_factor) * limit_sell + step_factor * min(marginal_costs, imbalance_penalty_price, limit_sell, max_price)
    elif self.df_bidders.at[player, 'x_imb'] < 0:
        limit_buy = (1 - step_factor) * limit_buy + step_factor * max(marginal_costs, imbalance_penalty_price, limit_buy)
...
def calc_volatility(transaction_prices, lambda_hat, i_max=5):
    ...
    volatility = np.sqrt(1/i * np.sum([(transaction_prices[-j] - lambda_hat) ** 2 for j in range(i)])) / (lambda_hat + 1e-5)
    return volatility
